...
void Renderer::render(float deltaTime, RenderGroupMask renderGroupMask) {
...
void Renderer::initFBAs() {
    ...
    for (size_t i = 0; i < swapchain.swapchainImageCount; ++i) {
        addSwapchainFBA(i, fbaIndices.depth);
...
void Renderer::initPipelines() {
    ...
    initBuffersAndTextures(standardAssetIndex, 
                           animatedStandardAssetIndex,
                           skyboxAssetIndex);
...
void Renderer::initBuffersAndTextures(size_t standardAssetIndex, 
                                      size_t animatedStandardAssetIndex,
...
void Renderer::initTextures(size_t standardAssetIndex,
                                size_t animatedStandardAssetIndex) {
...
void Renderer::createStandardAndShadowPipelines(size_t standardAssetIndex, size_t standardUboIndex,
                                                size_t shadowmapUboIndex, 
...
void Renderer::createSkyboxPipeline(size_t assetIndex, size_t uboIndex) {
...
int Renderer::createStandardPipeline(size_t assetIndex, size_t uboIndex,
...
int Renderer::createShadowmapPipeline(size_t assetIndex, size_t uboIndex,
...
void Renderer::bindAssets(Model const * models, size_t nModels,
...
void Renderer::update(prt::vector<glm::mat4> const & modelMatrices, 
...
void Renderer::updateUBOs(prt::vector<glm::mat4> const & modelMatrices, 
                              prt::vector<glm::mat4> const & animatedModelMatrices,
                              prt::vector<glm::mat4> const & bones,
                              Camera & camera,
                              SkyLight  const & sun,
...
void Renderer::updateSkyboxUBO(Camera const & camera, SkyLight const & sky) {
...
void Renderer::updateCascades(glm::mat4 const & projectionMatrix,
...
void Renderer::loadModels(Model const * models, size_t nModels, 
...
void Renderer::loadTextures(size_t staticAssetIndex,
                                size_t animatedAssetIndex,
...
void Renderer::loadCubeMap(prt::array<Texture, 6> const & skybox, size_t assetIndex) {  
...
void Renderer::createSkyboxDrawCalls() {
    GraphicsPipeline & pipeline = getPipeline(pipelineIndices.skybox);
    DrawCall drawCall;
...
void Renderer::createModelDrawCalls(Model const * models, size_t nModels,
                                    ModelID const * staticModelIDs,
                                    size_t nStaticModelIDs,
                                    ModelID const * animatedModelIDs,
                                    size_t nAnimatedModelIDs,
...
void Renderer::createCompositionDrawCalls(size_t pipelineIndex) {
    GraphicsPipeline & pipeline = getPipeline(pipelineIndex);
    DrawCall drawCall;
    drawCall.firstIndex = 0;
...
void Renderer::createVertexBuffers(Model const * models, size_t nModels,
...
void Renderer::createIndexBuffers(Model const * models, size_t nModels, 
                                      size_t staticAssetIndex, size_t animatedAssetIndex) {
    Assets & staticAssets = getAssets(staticAssetIndex);
...
void Renderer::createCubeMapBuffers(size_t assetIndex) {
    prt::vector<glm::vec3> vertices;
...
void Renderer::pushBackShadowFBA() {
...
size_t Renderer::pushBackDepthFBA() {
...
void Renderer::pushBackAccumulationFBA() {
...
void Renderer::pushBackRevealageFBA() {
...
void Renderer::pushBackSceneRenderPass() {
    ...
    scenePass.clearValues.resize(7);
    ...
    scenePass.clearValues[6].depthStencil = { 1.0f, 0 };
    ...
    // gui depth
    scenePass.attachments[6].format = findDepthFormat();
    scenePass.attachments[6].samples = VK_SAMPLE_COUNT_1_BIT;
    scenePass.attachments[6].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
    ...
    scenePass.attachments[6].initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
    scenePass.attachments[6].finalLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
    ...
    scenePass.subpasses[3].depthReference.attachment = 6;
...
void Renderer::pushBackShadowRenderPass() {
    // push back new render pass
...
void Renderer::pushBackSunShadowMap() {
...
prt::vector<VkPipelineColorBlendAttachmentState> Renderer::getOpaqueBlendAttachmentState() {
...
prt::vector<VkPipelineColorBlendAttachmentState> Renderer::getTransparentBlendAttachmentState() {
    prt::vector<VkPipelineColorBlendAttachmentState> colorBlendAttachments;
...
prt::vector<VkPipelineColorBlendAttachmentState> Renderer::getCompositionBlendAttachmentState() {
    prt::vector<VkPipelineColorBlendAttachmentState> colorBlendAttachments;
...
prt::vector<VkPipelineColorBlendAttachmentState> Renderer::getShadowBlendAttachmentState() {
    prt::vector<VkPipelineColorBlendAttachmentState> colorBlendAttachments;
